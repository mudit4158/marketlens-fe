...
export default function Chart3Indexed({ data, tz = 'IST', isLive = false, range: rangeProp }) {
  ...
  const mcxLabel = commodity === 'silver' ? 'MCX Silver (Indexed)' : 'MCX Gold (Indexed)';
  ...
  const opts = {
    ...
    scales: {
      ...
      y: { grid: { color: 'rgba(26,46,80,0.6)' }, ticks: { color: '#5A7090', font: { family: "'JetBrains Mono'", size: 10 }, callback: v => v.toFixed(1) } },
    },
  };
      ...
      <div className="card-head">
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div className="card-title">📉 Indexed to 100: Relative Performance</div>
            <InfoTooltip text={`All three series rebased to 100 at the start of the selected period.\n\nDivergence between MCX and COMEX lines reveals the USD/INR contribution:\n• If MCX rises more than COMEX → rupee depreciation is amplifying returns\n• If they move in lockstep → price move is driven by COMEX, not forex\n\nFormula: Indexed value = (current / first) × 100`} />
          </div>
        </div>
